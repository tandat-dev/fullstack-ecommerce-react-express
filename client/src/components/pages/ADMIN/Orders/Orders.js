...
function Orders() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (action === "edit") {
        await request.put("/editOrders", formData);
        handleClose();
        setReload(!reload);
      } else if (action === "delete") {
        await request.delete("/deleteOrders", {
        ...
        <nav>
          <ul className="flex gap-x-3 items-center">
            <li className="text-gray33 opacity-50 font-extralight text-base">
              <Link to={routesConfig.routes.dashboard}>Home</Link>
            </li>
            <li>/</li>
            <li className="font-extralight text-base">
              <Link to={routesConfig.routes.orders}>Orders</Link>
            </li>
          </ul>
          <ul>
                ...
                data.map((item) => (
                  <tr key={item.id} className="bg-white border-b">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex">
                        <p
                          className={`min-w-[110px] text-center py-1 rounded-full text-white uppercase ${
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-between">
                        <Link to={`/admin/orders/details/${item.id}`}>
